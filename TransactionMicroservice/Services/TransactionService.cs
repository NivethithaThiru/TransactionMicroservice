...
namespace TransactionMicroservice.Services
{
    public class TransactionService : ITransactionService
    {
        ...
        public async Task<TransactionStatus> Deposit(int accountId, double amount)
        {
            ...
            HttpResponseMessage response1 = httpClient.GetAsync("https://localhost:44359/api/account/getAccount?accountid=" + accountId).Result;
            if (response1.IsSuccessStatusCode)
            {
                ...
                if(account != null)
                {
                    ...
                    var message = new HttpRequestMessage(HttpMethod.Post, "https://localhost:44359/api/account/deposit?accountid=" + accountId + "&amount=" + amount);
                ...
                if(account != null)
                {
                    ...
                    if (account.CurrentBalance > amount)
                    {
                        ...
                        if (response2.IsSuccessStatusCode)
                        {
                            ...
                            {
                                HttpRequestMessage message = new HttpRequestMessage(HttpMethod.Post, "https://localhost:44359/api/account/withdraw?accountid=" + accountId + "&amount=" + amount);
        ...
        public async Task<IEnumerable<Financial_Transactions>> GetTransactions(int customerId)
        {
            HttpClient httpClient = _httpClientFactory.CreateClient();
            httpClient.BaseAddress = new Uri("https://localhost:44376/api/transaction");
            HttpResponseMessage response1 = httpClient.GetAsync("https://localhost:44359/api/account/GetCustomerAccountDetails?customerId=" + customerId).Result;
